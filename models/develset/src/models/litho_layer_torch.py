...
LastEditTime: 2021-04-14 19:48:08
...
class CUDA_LITHO:
    ...
    def computeImage(self, kernels, mask, dose):
        ...
        mask_cplx = torch.fft.fftshift(torch.fft.fft2(mask_cplx))
        ...
        # dtype = cfloat
        # km_cplx_real = torch.zeros((mask_cplx.shape[0], kernel_nums, mask_cplx.shape[2], mask_cplx.shape[3]))
        # km_cplx_imag = torch.zeros((mask_cplx.shape[0], kernel_nums, mask_cplx.shape[2], mask_cplx.shape[3]))
        # km_cplx = torch.complex(km_cplx_real, km_cplx_imag).cuda()
        km_cplx = torch.zeros((mask_cplx.shape[0], kernel_nums, mask_cplx.shape[2], mask_cplx.shape[3]), dtype=torch.cfloat).cuda()
        ...
        km_cplx = torch.fft.ifft2(km_cplx)
        real = km_cplx.real
        imag = km_cplx.imag
        sim_ri = (real*real + imag*imag)
        sim_ri = sim_ri * kernels['kernel_scale']
        I = torch.sum(sim_ri, axis=1, keepdims=True)
    ...
    def convolveKernel(self, mask, kernel_type, dose):
        ...
        # print('kernel device:', kernels['kernel_head'].device)
        # print('mask device:', mask.device)
    ...
    def convolveKernelCpx(self, kernels, mask, dose):
        if mask.dtype is torch.float32:
            mask_cplx = self.img2cplx(mask, dose)
            # mask_cplx = self.tensor2cupy_cpx(mask_cplx)
            mask_cplx = torch.fft.fftshift(mask_cplx)
            mask_cplx = torch.fft.fftshift(torch.fft.fft2(mask_cplx))
        else:
            mask = torch.fft.fftshift(mask)
            mask_cplx = torch.fft.fftshift(torch.fft.fft2(mask))
        kernel_nums, kernel_w, kernel_h = kernels['kernel_head'].shape
        # print('convolveKernelCpx use {} kernels'.format(kernel_nums))
        km_cplx = torch.zeros((mask_cplx.shape[0], kernel_nums, mask_cplx.shape[2], mask_cplx.shape[3]), dtype=torch.cfloat).cuda()
        imxh = IMAGE_HEIGHT_WIDTH//2
        imyh = IMAGE_HEIGHT_WIDTH//2
        xoff = imxh - kernel_w//2
        ...
        km_cplx[:, :, xoff: xoff+kernel_w, yoff: yoff+kernel_h] = (
        mask_cplx[:, :, xoff: xoff+kernel_w, yoff: yoff+kernel_h] * kernels['kernel_head'])
        km_cplx = torch.fft.fftshift(km_cplx)
        km_cplx = torch.fft.fftshift(torch.fft.ifft2(km_cplx))
        return km_cplx
...
class LEVELSET_LAYER(torch.autograd.Function):
    ...
    def forward(ctx, mask, target, cl, cfg):
        ...
        outer_aerial, outer = cl.simulateImageOpt(mask, LITHO_KERNEL_FOCUS, MAX_DOSE)
        inner_aerial, inner = cl.simulateImageOpt(mask, LITHO_KERNEL_DEFOCUS, MIN_DOSE)
        levelset_loss = ilt_weight * cl.l2_loss(nominal, target) + pvb_weight * (cl.l2_loss(inner, target) + cl.l2_loss(outer, target))
        # levelset_loss = cl.l2_loss(nominal, target)

        #calculate d (Z0-Z^)^2========================
        tc_nominal = (nominal-target) * nominal * (1 - nominal)
        # print('tc_no device: ', tc_nominal.device)
    ...
    '''
    TODO:
    it is not right to add the pvband_coef directly
    '''
    @staticmethod
    def backward(ctx, levelset_loss, aerials, wafers):
        mask, d1_nominal, d1_defocuse = ctx.saved_tensors
        ilt_weight, pvb_weight = ctx.ilt_weight, ctx.pvb_weight
        gradient = PHOTORISIST_SIGMOID_STEEPNESS * (ilt_weight * d1_nominal.real + pvb_weight * d1_defocuse.real)
