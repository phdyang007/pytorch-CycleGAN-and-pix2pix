...
LastEditTime: 2021-04-14 21:45:33
...
class Develset(pl.LightningModule):
    ...
    def cal_prp_coef(self):
        # sum1, sum2, sum3 = 0, 0, 0
        tmp1 = self.mask.grad * self.mask.grad
        tmp2 = self.mask.grad * self.mask_grad_last
        tmp3 = self.mask_grad_last * self.mask_grad_last
        sum1 = torch.sum(tmp1)
        sum2 = torch.sum(tmp2)
        sum3 = torch.sum(tmp3)
        return (sum1 - sum2)/sum3

    def cal_vel(self, epoch):
        if epoch == 0:
            self.vel = self.mask.grad
        else:
            eta = self.cal_prp_coef()
            self.vel = self.mask.grad - eta * self.mask_grad_last
        # self.vel = vel
        return self.vel
    ...
    def training_step(self, batch, batch_idx):
        if self.current_epoch == 0:
            ...
            self.mask_grad_last = torch.zeros_like(self.mask)
            # self.__updateMask()
        ...
        if self.mask.grad is not None:
            vel = self.cal_vel(self.current_epoch)
            time_step = 2.5 / torch.max(torch.abs(vel))
            ...
            if self.cfg.model.add_curv:
                ...
                self.ls_params.data.add_(time_step * (vel * gradient_geo + curv_weight * curv_term))
            else:
                # self.ls_params.data.add_(time_step * self.mask.grad * gradient_geo)
                self.ls_params.data.add_(time_step * vel * gradient_geo)
            # truncated levelset
            ...
            # momentom
            self.mask_grad_last = self.mask.grad.detach()
