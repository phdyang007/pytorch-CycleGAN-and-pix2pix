...
class oinnopcg(nn.Module):  
    def __init__(self, subtile_size = 64, dilated=3):
        super(oinnopcg, self).__init__()
        ...
        self.diconv = nn.Conv2d(in_channels=self.width, out_channels=self.width, kernel_size=self.dilated, padding = self.subtile_size, dilation=self.subtile_size)
        #refine
        self.convr0 = nn.Conv2d(in_channels=self.vgg_channels[5], out_channels=self.refine_channel, kernel_size=self.refine_kernel, padding = (self.refine_kernel-1)//2)
        self.convr1 = nn.Conv2d(in_channels=self.refine_channel, out_channels=self.refine_channel//2, kernel_size=self.refine_kernel, padding = (self.refine_kernel-1)//2)
        self.convr2 = nn.Conv2d(in_channels=self.refine_channel//2, out_channels=self.refine_channel//2, kernel_size=self.refine_kernel, padding = (self.refine_kernel-1)//2)
    ...
    def forward(self, x):
        ...
        #print(x_fno.shape)
        n,c,h,w= x_fno.shape
        tile_c = h//self.subtile_size
        #print(n,tile_c*tile_c,c,self.subtile_size,self.subtile_size)
        #quit()
        x_fno_input = torch.zeros((n,tile_c*tile_c,c,self.subtile_size,self.subtile_size)).type('torch.cuda.FloatTensor')
        x_fno_output = torch.zeros((n,self.width,self.subtile_size*tile_c,self.subtile_size*tile_c)).type('torch.cuda.FloatTensor')
        for i in range(tile_c):
            for j in range(tile_c):
                x_fno_input[:,i*tile_c+j,:,:,:] = x_fno[:,:,i*self.subtile_size:(i+1)*self.subtile_size,j*self.subtile_size:(j+1)*self.subtile_size]
        

        


        x_fno = self.fno(x_fno_input.view(n*tile_c*tile_c,c,self.subtile_size,self.subtile_size))       
        x_fno = self.act_fn(x_fno) #n*t*t, 16, 64, 64
        
        x_fno = x_fno.view(n, tile_c*tile_c, self.width, self.subtile_size, self.subtile_size)
        #print(x_fno.shape)
        for i in range(tile_c):
            for j in range(tile_c):
                #print(x_fno[:,i*tile_c+j,:,:,:].shape)
                x_fno_output[:,:,i*self.subtile_size:(i+1)*self.subtile_size,j*self.subtile_size:(j+1)*self.subtile_size] = x_fno[:,i*tile_c+j,:,:,:] #n c h w
                #quit()
        x_fno = self.diconv(x_fno_output)
...
class oinnopcgv2(nn.Module):  
    def __init__(self):
        super(oinnopcgv2, self).__init__()
